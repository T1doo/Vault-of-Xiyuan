...
当前GPU2正在执行2,000步全数据S开发训练（run_id=$XIYUAN_RUN_ID，PID（见本机记录），工具session（见本机记录））；最新核对约1043次有效更新，已保存第500、1000步完整checkpoint。第1000步固定训练帧有4/5次自然生成合法，但20个预留clean闭环单元均因后续动作解码失败结束。继续训练、候选解码排查与多checkpoint闭环。负责人代表回放/控制语义与曲线审阅仍待完成。F1不会因loss下降或单个checkpoint存在而自动通过G1。
