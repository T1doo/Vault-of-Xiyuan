...
2,000步全数据S开发训练已完成并完整保存500/1000/1500/2000步checkpoint；最终2000步固定训练帧4/5自然生成合法，20个预留clean闭环单元为2成功、1超时、17次显式解码失败。当前已启动同一研究范围内的3,000步独立开发延长，使用全量数据和相同batch/严格解码；G1仍未通过，负责人代表回放/控制语义与曲线审阅仍待完成。
