...
当前GPU2正在执行2,000步全数据S开发训练（run_id=$XIYUAN_RUN_ID，PID（见本机记录），工具session（见本机记录））；最新核对约595次有效更新，已保存第500步完整checkpoint。第500步固定训练帧的候选位置推理仍报告动作系数长度错误；第300步小样本模型五个预留开发初态也全部失败。继续训练、候选解码排查与多checkpoint闭环。负责人代表回放/控制语义与曲线审阅仍待完成。F1不会因loss下降或单个checkpoint存在而自动通过G1。
