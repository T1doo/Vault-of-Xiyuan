...
本文放在 `docs/CODEX_EXECUTION_GUIDE.md`；配套人类阅读计划为 `docs/EXPERIMENT_PLAN.md`，当前服务器的工作规则位于仓库上一级 `$XIYUAN_WORKSPACE/AGENTS.md`（不纳入本公开仓库）。本文包含实施所需的项目背景、数据与模型定义、实现接口、任务顺序、验收测试、资源预算和分阶段安排，不依赖原聊天才能理解范围。
...
本基础阶段做完四组：S=π₀-FAST；A=项目书 2.2 的显式空间提示；B=项目书 2.3+2.4 对应的隐式三维对齐与三维教师；完成仿真验证。已只读查见宿主机八张 RTX A6000 48GB，多人共享；负责人已于2026-09-07明确授权“只要有空的就可以用”：所有确认空闲的GPU均可使用，不设两卡上限，可增加GPU数量及相应资源预算、并行推进依赖已满足的任务；超过两卡无需再次询问。每次启动前检查占用并统一登记，按实际空闲卡数分配。负责人确认无硬截止日、无已有项目环境/数据，由 Codex 自行下载与准备；正式长作业仍遵守 gates/批准队列，空闲不足先等待或单卡推进。
...
空闲GPU可全部投入项目，不设固定两卡并发上限。默认一个大作业一张卡，将不同的已就绪作业分配到不同空闲卡并行；worker渲染显存也计算在内。空闲卡及卡时预算增加已有负责人授权，排程按实际资源更新。以 `CUDA_VISIBLE_DEVICES` 限定可见卡，注意进程内部逻辑编号可能变为 0，不能把它与物理编号混淆。不得终止不属于本项目的进程。

**资源分配口径：**“只要有空的就可以用”是持续授权，不必为第3张及以后的空闲卡另行确认。按项目全局登记GPU UUID、作业、卡时和停止条件，防止多个会话重复认领；优先并行独立任务。增加单个作业的data parallel/FSDP卡数时，需入口已实现并验证，保持有效batch/更新数/样本协议；资源授权本身不替代科学gate或扩充研究支路。
...
**F2 审阅方式补充（2026-09-11）：**负责人可明确采纳固定 300 题的独立 GPT 图文技术复核作为“AI 辅助技术质检”证据，记录 `gpt_technical_reviewed=300`、真实限制和 `human_reviewed=0`，不得改写为真人盲审或报告真人一致率。该方式可以支持当前规则采用和后续工程 pilot；任何明确要求真人证据的交付或 gate 仍保留 `BLOCKED_HUMAN`，直到真人记录补齐。
...
**输入：**当前仓库/服务器。**动作：**只读检查 Git/GPU/环境/数据/权重/占用；确认环境隔离、当前空闲GPU及全局分配、既有脚本与旧结果。
...
主训练结束后可用空闲GPU主要用于并行评测，数量不限两卡。使用固定 shard 和 lease 防止重复调度；动态 worker 并发只用于吞吐，不改变输入、控制/动作块长或超时标准。
...
G2 后按训练+评测+后段 70 卡时缓冲估算：Lite 630、P1 770。两卡 70% 持续可用时约 18.8/22.9 个自然日等效资源占用，不含前置开发与后续人工交付，也不是无条件周期承诺。30日两卡70%的理论1,008卡时仅作历史算例；当前授权不限两卡，预算可增加，按实际空闲GPU数量及窗口重算。
