...
版本日期：2026-09-06。基于 V2 与审阅意见修订；按 F0—F6 阶段和验收证据推进，可跨多次会话暂停/续接，不要求负责人每天在线。约一个月仅为资源估算参考。本文是工程任务合同与实验背景，不是已完成代码或服务器实测记录。
...
本基础阶段做完四组：S=π₀-FAST；A=项目书 2.2 的显式空间提示；B=项目书 2.3+2.4 对应的隐式三维对齐与三维教师；完成仿真验证。服务器有 Codex 辅助，磁盘充足；GPU 暂按两张 RTX A6000 48GB 规划，但型号、数量和占用需审计。
...
repo/
  ...
  docs/
    ...
    STATUS.md                         # 待创建：事实状态，不填虚假完成
    DECISIONS.md                      # 待创建：语义与协议决定
    RUNBOOK.md                        # 待创建：实际验证过的命令
...
以下为可直接用于目标服务器只读审计的系统命令；尚未在用户服务器执行：
