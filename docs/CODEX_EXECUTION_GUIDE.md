...
版本日期：2026-09-06，V3 审阅补充版，采纳范围见 `docs/DECISIONS.md`。按 F0—F6 阶段和验收证据推进，可跨多次会话暂停/续接，不要求负责人每天在线。无硬截止日，约一个月仅为资源估算参考。本文是工程任务合同与实验背景，不是已完成代码或模型实测记录。
...
原项目书不修改，也不要求先返修申请书。执行中的纠错、具体化和范围差异写入本文、实现及 `docs/DECISIONS.md`。本版文档版本不自动改写已有正式协议/结果；检查已有工作后决定继续原协议或显式开启新版本。
...
2. 读取本手册与 `EXPERIMENT_PLAN.md`。如果它们未按推荐目录放置，先定位实际文件，不创建空文件冒充。
...
默认在 openpi 工作副本根目录增加项目代码，不重建整个上游仓库。已有工程布局优先兼容；若适配布局，同步文档与路径配置。

下列是未来工程结构示意。当前权威规则在工作区根 AGENTS.md；状态/决策/命令账本已在 Vault-of-Xiyuan/docs，不在工程 checkout 复制第二份。四个已下载的上游源码位于工作区 upstream/，尚未改造为项目实现；其存在不代表环境、数据或模型已验证。

```text
repo/
  # 使用工作区上级 AGENTS.md，不复制进公开仓库
  docs/
    EXPERIMENT_PLAN.md
    CODEX_EXECUTION_GUIDE.md
    STATUS.md                         # 当前已存在于权威文档仓库
    DECISIONS.md                      # 当前已存在于权威文档仓库
    RUNBOOK.md                        # 当前已存在于权威文档仓库
  src/openpi/                         # 上游，最小必要修改
  src/xiyuan/                         # 待创建
    data/                             # schema、manifest、标签、cache reader
    models/                           # A 序列；B projector/loss；模型适配
    training/                         # batch、损失汇总、累计、metrics
    evaluation/                       # request schema、manifest、结果检查
    analysis/                         # 聚合、配对统计、成本
  scripts/xiyuan/                      # 待实现 CLI，不能假装已存在
  configs/xiyuan/
    common.yaml
    s.yaml
    sa.yaml
  ...
  manifests/
    ...
    val.jsonl
    dev_eval.jsonl
    final_eval_full.jsonl             # 核心与已登记扩展的去重并集
    final_eval_core.jsonl
    final_eval_extension.jsonl        # 无合法扩展时不启用
    teacher_train.jsonl
  artifacts/
    audits/
    qc/
...
大数据、教师缓存、权重与视频可放用户现有数据盘，用 `paths.local.yaml` 指向绝对路径。不要硬编码本次聊天的 `/mnt/data/...` 路径为服务器路径。
...
每次会话结束更新已做/未做事项。`DECISIONS.md` 记录日期、问题、依据、选项、采用方案、适用模型、是否需要重建缓存/重训/重评。`RUNBOOK.md` 只收录真正跑过且结果可定位的命令；模板留在本手册，不混称已验证命令。
...
以下为只读审计命令示例；本机已执行的具体查询、结果和退出码见 RUNBOOK。默认沙箱可能不暴露 GPU/宿主机进程，失败时先核实执行环境，不判定驱动损坏；不能凭沙箱 ps 判断整机空闲。
...
| `scripts/train.py`、`scripts/serve_policy.py` | 训练和策略服务入口 | 核验参数名后再写 RUNBOOK |
...
项目书未固定这一坐标系、槽位与阈值；这些写入 `direction_protocol.json` 和 `DECISIONS.md`。F2 先确定并质检可执行规则，G2 后不能按最终语言/视角测试失败临时修改。
...
T03 追加无需训练的每槽位多数类参照：由 train 有效标签确定类别、固定并保存 train/规则 hash，在独立开发数据的相同有效集合上报告参照与模型准确率、混淆矩阵、逐类召回率和覆盖；并列多数类的确定性处理写入规则，无支持项明确标记。不得用开发/最终标签重选多数类。GT 阶段参照仅离线诊断，不作公共输入公平基线。偏斜不是失败 gate，不自动加类别权重、重采样或删类。
...
T00—T12 保持原编号；阶段对应 F0—F6，不对应某个自然日。任务在产物与测试存在时才设 DONE。独立分支/worktree 开发；公共接口先统一并由集成人合并。以下路径均为拟新增，实际名称改变时同步 RUNBOOK。

### T00：现状、硬件、版本与资源盘点（F0）

**输入：**当前仓库/服务器。**动作：**只读检查 Git/GPU/环境/数据/权重/占用；确认环境隔离、两卡假设、既有脚本与旧结果。

**输出：**`artifacts/audits/hardware.json`、`versions.json`、`paths.json`、STATUS/DECISIONS 初始账本及现有产物复用清单。
...
**输出：**最终 RUNBOOK、代码/权重/配置哈希、数据/缓存/协议/结果索引、复现报告、结论边界和真实未完成清单；G5。
...
读取上级/本地规则、Git 状态和三份本项目文档相关章节；检查 STATUS、DECISIONS、RUNBOOK 的最新记录。先看已在运行的 PID/心跳、已完成 checkpoint/shard 和协议哈希，防止重复启动。已有通过且依赖/版本未变的测试可引用证据，不机械全量重跑。
...
先验证脚本存在并运行 `--help`，再写命令；自定义 schema 字段不自动等于上游 CLI 参数。未实现的脚本只登记为 planned，不写进已验证 RUNBOOK。
...
`name, script_path, status(planned|implemented|verified), verified_commit, help_log, smoke_command, smoke_exit_code, required_environment, required_gate`。runner 只执行 implemented 且通过必要 smoke 的入口，正式长作业还要满足相应 gates。
...
四组实际完成的所有 seed 权重与同 K 证明；基础/adapter/投影头的元数据与哈希；固定代码/环境；共享数据/归一化/动作变换；方向角色/实例/标签及阶段覆盖、教师缓存的版本/QC；真实 freeze/更新/内部预处理/严格解码证据；P0/P1-Lite/P1 实际采用范围；官方资源库存、唯一单元/关联簇和所有正式 eval_id 的有效结果或缺失解释；四组与七维表；配对增益、交互、G_drop 和冻结的重采样方案；资源实测；失败视频；无监督目录推理验证；从新进程恢复的 RUNBOOK。
