...
2026-09-19当前节点：SA-full/SB-full在前100步预算校准后继续运行，但最新现场显示两条训练进程均已停止写入，未形成1000步checkpoint。首次异常快照为SA=282、SB=236；随后run文件又推进到最后观察到的SA=288、SB=241，之后heartbeat不再更新，不能把历史RUNNING字段当作活跃状态。当前节点已标记为GPU访问异常暂停，F2仍未验收，F3/SAB未授权；不在GPU设备恢复前重启或创建第二写入者。最近完整磁盘checkpoint须以实际保存节点为准，不能把heartbeat当恢复点。

2026-09-19最新：第九参照精确通过，固定输入服务回归补齐；累计GPU回归110.2785秒已计SA34。两组已在两张空闲A6000统一启动，实际初始化及首3更新均核对通过，正在同run前100步预算校准。下面“尚未启动/回归缺口”均保留为各自历史节点。

2026-09-19第九次补充参照已通过：val591同SA step-3000输入/生成token/动作/严格解码与先前真实回退精确一致，raw dtype为float32。累计服务GPU成本110.2785秒计入SA34，服务已退出；原8请求缺口现已补齐，旧失败与dtype证据限制不改。两组统一启动前检已完成CPU身份和runner预算控制检查，接下来按原许可启动实际初始化与首100步校准。

2026-09-19服务修复本批结束位置：CPU11项通过，累计GPU/WebSocket请求8次（1次失败留证、7次返回）；两个正常路径对照和首输入回退对照精确一致，第二输入缺direct action-only参照，完整回归仍未通过。新增GPU占用89.2777秒计入SA34；两组训练均未启动，保留原reader采用及34/42/76预算。下面“回归运行中”属于本批早先记录。

2026-09-19补充批复已采纳：实际SA回退与审计修复已实施，9项CPU分支测试通过；有限真实GPU/WebSocket回归已启动，尚未宣布通过。回归≤1卡时计入SA34；两组训练仍未启动。SB原reader采用保持，原阶段限制不变。本机旧源码与CPU/GPU记录在既有审计目录sa-runtime-fix-v1，生产入口无注入开关。

2026-09-19本批检查结束：SB采用未改原reader，原件计时及完整schedule命中分析已完成；新预算SA34/SB42/总76有效，前100步校准未运行。启动前对SA真实服务分支的CPU synthetic复现发现方向超预算没有action-only重新prefill；两组均未启动，新增GPU成本0。当前待处理是该具体服务缺口，不是原类型适配、QC或reader故障。

2026-09-19：完整读取负责人指定分享链接交接，已采纳“先SB读取检查与采用决定，后两组统一启动”。新预算SA34/SB42/总76 GPU小时；旧预算前检保留历史。当前只开始有界CPU检查，两组未启动。文档基线6602762，生产入口/reader快照及2小时截止时间留既有审计目录reader-first-v1；不修改运行中依赖，F2未验收，F3/SAB未授权。

2026-09-18最新批复：SA-full/SB-full方案、五项基本功能判据及每组30/合计60卡时上限已批准，逐组预算可行方可启动。两组CPU监督连接通过；保守预算分别约31.50/37.24卡时，当前均未启动，GPU初始化/更新/诊断/闭环均为0。按批复先报告预算差额，不自动扩额或缩减范围。F2仍未验收，F3/SAB未授权。下方历史“方案待审”状态由本次批准记录取代。

2026-09-18本轮更新：S-full执行完成已获认可，F2仍未验收。本轮仅完成原件CPU复算及SA-full/SB-full匹配验证计划，未启动GPU、训练或仿真。下一步为技术审阅F2 PLAN末尾的功能操作化判据与每组30卡时预算；计划通过前不执行。
...
执行上限为同一张实测空闲卡30 GPU小时，包含初始化核对、训练、保存、1000/2000/3000各固定10train+10val自然生成/TF及终点原20clean回合。编译缓存从起步启用既有私有实现；SB历史1001才启用的时间边界单列，已有缓存执行等价证据保留，不把吞吐差算模型效应。到预算前保存最近完整更新并停止，不自动延时；无正式结果或阶段自动放行。

S-500已停止，1000/2000/3000完整保存与三点诊断、终点20clean均完成；本批新增S-full授权仅限一个单卡30小时对照，启动状态以本机新run编排为准。

此前中期诊断实际60/80生成、6/8前向，SA1000缺口保留，本轮不补；本S对照结束只回答匹配问题，不自动通过F2，不进入F3。GPT技术复核采用与human_reviewed=0、独立备份未知限制不变。
...
### 2026-09-16｜批准并启动匹配SB的S-500（正在运行）

负责人基于中期诊断批准一个S-500匹配参照：同SB的base、LoRA初始化流、原500池及采样顺序、microbatch4×累计4、144容量、优化器/3000步LR日程，只结构关闭B（且无A）；最多单卡30 GPU小时，含三个时点同20观测自然生成/TF及终点原20clean回合，不训练SA/SAB、不补SA1000、不进入F3。执行前基线文档`9692c51710161151197a7744c9bb2e59bcc27078`与已有未提交工作核对，保留SB原入口快照和所有旧结果。

**实际入口改动。** 同一`ab_training_entry.py`增加real模式`s`分支，动作batch直接使用ProjectSPipeline.training_batch，teacher target/valid为None、direction mask全零；`_loss_with_public_model`只返回动作loss，B层捕获/投影头/对齐梯度不执行。显式传入方向/教师依赖的S配置被拒绝，未把SA的缺标回退当S训练。初始化、稳定schedule、AdamW、累计和联合裁剪函数与原SB固定源码AST逐项相同；只补真实截止时间安全保存与完成更新计时，不改变数学日程。

真实入口SHA256=`1707fd325296470b24eeffca7858f3cb0b1da10c32c17cd523e6725041bb14a5`；原SB入口SHA仍`b076a77cf5e96bf0544468892feea13833829c12941a84207ecfc2ef48332579`并保留快照。S绑定自己的resolved-config/source hash，不关闭原SB恢复身份检查，不重写旧run配置。

**训练前CPU证据（退出0）。** 全3000×16样本顺序与原SB metrics逐行一致，schedule SHA=`d503545a9c5e6e549bae3e347fa674cf970dc10ffa23204f25c34007e1ed3b50`；500池、seed0、batch4×4、144、base/norm/tokenizer/动作合同及AdamW/LR字段一致。teacher_reader/directions均None，S微批次teacher target/valid均None，方向mask=0、动作监督非空。仅variant、辅助开关/来源、实现hash和显式manifest池声明等预期字段不同，不改变真实sample_id集合或顺序。

**实际初始化（首次更新前）。** 用未变的key101初始化函数创建S，并从归档SB原始入口独立重建公共初始化（不创建B投影头）。10个LoRA数组叶子及全部模型参数精确一致，S初始LoRA fingerprint=`2f86409ca081f3a222e1fe652ffd2de6e833927e1e4e43b8aa9ea3223259ac1a`。该验证是按实际SB执行源码/同配置重建，而非声称历史未保存的SB初始数组原件仍存在。训练采样RNG仍key0，不混同F1的fold_in初始化流。参考对象比较后释放，正式S只保留一套学生训练状态。

**启动更新证据。** 已核验前3步：动作loss为14.703125、13.828125、14.6875，direction/alignment均None；第一步原sample_id和动作loss与SB第一步精确一致。LR约2.99697e-8、5.99393e-8、8.99090e-8；真实共享LoRA更新且冻结指纹不变。第1步36.336秒含新编译，第2/3步26.862/25.940秒，不把三步当长程吞吐保证。首几步直接计入同一次3000预算，未额外短训或重启。

**顺序执行与停止。** 单个runner同卡执行训练，原1000/2000/3000完整保存后用对应模型参数做固定10train+10val：每点20自然生成、2次batch10 TF，全部只走公共S动作路径；原选择与SB相同，诊断不更新权重，也不读取A标签/教师缓存。诊断后继续原有效更新/采样位置，不重置optimizer/scheduler。終点3000后独立加载实际S服务，原20clean任务/row/初态、220最大步、10预热、执行5/预测10×7不变，保存raw token、具体错误及回合/请求关联，所有失败纳入，不挑checkpoint。

编译缓存采用原私有SB缓存目录及已验收设置，从S第1步启用；SB历史前1000未启用而1001起启用，此为已证明数学不变的执行优化边界，不是样本或优化器条件变化，也不据此比较整程GPU效率。生成非法是被测结果，不重试至合法；训练非有限/冻结参数变化/缓存或身份错误停止受影响run留现场。

总deadline为启动后108000秒。训练预留末尾15分钟供终点评测/退出，并在预计剩余不足180秒时保存最新完整更新（已存在完整checkpoint不覆盖）写PAUSED_BUDGET；强制最终时限保护不扩大预算。若任何诊断因预算未完成，明确保留未执行部分，不伪造三点齐全；不因没有达到3000自动加卡时。所有初始化核对、编译、诊断、保存及评测纳入总账，当前未声称这些未来阶段已经运行。
...
其中每个百分比的分母是固定的10个观测或对应TF有效token；非法模型输出保留具体状态，未补零、截断或重试。step-3000 诊断元数据 SHA256=`07ec5b8fc0ab5a68324f22021ef29bfed36086ef55d3589ca1fd40ec813fca22`；其 `requests.jsonl` 中保留完整 raw token、边界、EOS、FAST token及实际/期望系数数。
...
### 2026-09-17｜S-full交接采纳与CPU前检

已完整读取负责人转发的新交接。起始文档ecfc04c4892b1f5e9ae5aa5c62288a3347f47fcf，工作树干净；实际共享入口hash与交接1707fd…一致。只读宿主核对无旧S-500或本项目评测作业，不触碰其他作业。

S-500原件复算：1—3000连续、48000位置/500唯一ID；20回合关联24条服务raw请求，4合法、18长度、2边界错误，累计20模型动作步；17回合首次请求失败、3回合至少一次合法后失败。与SB三时点sample_id集合精确对应；S-500 val从1000到3000为1合法保持、7合法转非法、0非法转合法、2持续非法，2000到3000为1/1/0/8。保留单seed、小开发探针限制，不能唯一归因数据量，也不能把共同失败全归因B。

共享入口仅新增显式external-schedule读取和manifest/hash/ID/shape/split校验；初始化、优化、累计、裁剪和保存函数AST与S-500原件相同。实际CPU loader核对完整55682候选及原池内/外样本、同ID打包精确一致、动作mask非空，监督读取守卫开启。外部schedule绑定配置及保存，恢复沿原完整schedule比较和有效cursor继续。完整schedule文件SHA256=dd0bcd1a9076c2ff3c23a6f9b57667552c61d2cd29c58533c1059357753f77ba。

固定视频规则为每任务row0全程保存，编码CPU smoke通过；只增加记录与预算退出检查，不改策略输入或成功判据。源快照、CPU配对/曝光/时间分箱、前检和后续runner位于本机既有解码审计目录的sfull-v1；旧产物只读，无新GPU模型计算截至本CPU节点。下一步为前检通过后的获授权单次启动，F2仍未验收。

启动记录：CPU前检通过后，已执行本机`sfull-v1/run.py`，新run为`<SFULL_RUN>`，原S-500未重启。编排状态RUNNING，正在加载/初始化，尚不声称首更新通过；实际PID/GPU/deadline在本机orchestration.json。唯一训练入口SHA256=d2b3bf09f474ad8a7793f5638d56d53ead0bdc18ce4e9419719ee6644cbe9692；帧采样源及48000位置在开训前固定。采样以外的数值函数未修改，视频与预算只用于新评测入口的显式可选参数。GPU总额包括加载/编译，预算从此次启动累计，不随重启重置。

### 2026-09-17｜S-full前100步预算校准

S-full已完成前100次有效更新，`result-metrics.jsonl`连续100行且sample_id逐位置匹配固定外部schedule；heartbeat仍为`RUNNING`。前100步更新耗时中位数约24.44秒，按该窗口估计纯更新约20.37 GPU·小时；这只是阶段估计，后续保存、三点诊断、终点评测和波动继续计入同一30 GPU·小时硬上限。当前不改batch、精度、采样或更新预算。

### 2026-09-17｜S-full初始化与前3次有效更新核验

实际共享LoRA初始指纹与S-500的2f86409c…一致，全部初始参数与归档S-500初始化实现重建对象精确一致；后者明确属于重建证据。首3步动作loss=14.953125/13.0625/14.25，耗时28.895/27.108/25.967秒；输入不同，不要求loss等于S-500。实际sample_id与新schedule逐位置一致，动作有效token非空、辅助项None，冻结不变/LoRA更新断言均在落盘前执行。公开数值配置除预登记的数据计划和源码身份外与S-500一致。单一runner仍存活，未形成1000步恢复点；100步窗口再估预算，不能用首3步声称整程benchmark。前检、初始化和startup-verification原件留本run，继续原3000目标，不改batch/精度或预算。

### 2026-09-17｜F2期间的F3前置CPU准备：限定授权登记

本批仅实施F2 PLAN第6节的无依赖前置准备：现有库存候选清单、结果统计CPU测试及有余力的静态配置/预算草案。S-full训练、保存、诊断与评测授权不变。F2未验收，F3主流程、SAB和G2冻结仍未授权。本批输出与真实运行数据隔离。

文档起点为`50e2149c5236a5201e1ab85bb9c3127d95b89cbe`，工程HEAD为`16295beccf737e1e180718fe78af963cd8707999`；已有Gemma层捕获dirty补丁和未跟踪规则入口保留，差异另存本机。唯一技术子目录为`<CPU_PREP>`，其中独立文档worktree由本会话串行发布；训练入口及稍后服务/评测重新导入的源码均只读，保护摘要及原始路径留`<CPU_PREP>/registration.json`。

本批GPU=0、一个主要CPU进程、线程最多2；分析进程使用更紧的2 GiB地址空间上限（授权总内存上限8 GiB）。只读既有库存/小型结果，不读权重、原图或教师payload，不运行仿真、模型、队列或持续监控。开始核对原训练进程存活；具体进度仅存本批带时间的只读快照，不改训练方维护的LOG顶部和run记录。以下执行结果待实际完成后追加。

### 2026-09-17｜F2期间的F3前置CPU准备交付

**范围与结论。** 本批A/B主交付及可选C/D已完成；F2=IN_PROGRESS／未验收，F3=TODO／主流程未启动，G1保持PASS，G2、SAB、四组400步/24 GPU小时和正式队列均未授权。仅修改本批独立技术目录和文档副本，未运行模型/仿真/新reset、未申请GPU、未改训练/服务源码、数据、环境、标签、教师或编译缓存。公开[脱敏测试回执](review/final-evidence/cpu-prep-20260917.json)与既有真实aggregates.json分开，后者及图表未改。

**隔离与版本。** 文档基线`50e2149c5236a5201e1ab85bb9c3127d95b89cbe`；工程HEAD`16295beccf737e1e180718fe78af963cd8707999`加既有Gemma dirty补丁（原件保持），不能用文档commit冒充执行代码。独立技术目录`<CPU_PREP>`含候选/工具/测试及唯一文档worktree；本会话是本批唯一发布者，合并前重新核对最新文档。运行中的S-full由原`sfull-v1/run.py`及其检查点回调、后续服务/客户端负责，本批无接管。保护的15个小型源码/配置开始与结束摘要一致，原工程dirty范围未改。完整版本、来源hash、调用方式及进程快照在本机registration和finish-check；私人路径/标识不发布。

**A：候选及用途审计。** 复用F1 Plus库存2,402配置、101,235条存储初态行；既有行hash在每配置内无重复，这不是独立reset证明。沿既有clean源行0—4开发预留，将其保守用于多行Plus配置；实际F1/SA/SB/S-500、S-full锁定开发单元和离线探针均另记来源。整理50个clean预留源单元、13个Plus已用源单元（合计63）及25个去重离线样本，另保留7项native-reset seed诊断；source-row开发用途与native reset不混成一种身份。原S-full未执行的固定20回合及固定探针同样保留开发用途。

选择仅依赖任务/配置字典序和既有资源，不读取成功率作筛选：clean及每任务每个非布局维度选择一个官方配置的源行5—24；布局选择同为92维状态的官方配置，按字典序最多20个，每配置唯一源行。共247个配置、1,577条核心候选。clean及六个非布局维度各200条，布局177条；按任务索引0—9为20/17/19/13/18/19/20/16/16/19，距每任务20条少23条。没有复制初态、换无作用seed凑数，也没有新增第二扩展包。

候选静态身份1,577个无重复；与登记开发源单元精确重合0，与已知开发源状态簇重合0。1,400条候选可依据相同源资产/存储向量关联到官方源状态，177条布局仍unknown；**全部1,577条与演示初态的关联仍未知**。这些结论不等于严格初态留出。旧评测`unit_fingerprint`实际只哈希初态数组，本批保留旧值的含义，不冒充完整单元标识。候选`unit_fingerprint`和`reset_hash`均为null，实际随机性、最终控制/成功判据版本和reset生效待核，因此“已验证正式有效单元数”为0；候选静态ID不冒充完整fingerprint。

本机`candidate-final.json`保留机器可读清单、全库存及所选配置的任务×维度×包×配置计数、开发用途、关联证据/unknown、原指令/hash和缺口；数量分类可能重叠，字段明确不相加。`source-receipts.json`记录输入内容身份。只对必要的小型稳定源文件/初态资产计算摘要，不反序列化状态资产、不读取权重/图像/教师payload。重复构建的排序、身份与全部统计精确相同；80个任务×条件的预设包内配置权重和为1。候选不在runner发现目录，不生成正式protocol ID或protocol.lock。

**B：统计工具与证据。** 已检查现有`final_check.py`、`functional_reassessment.py`及公开渲染汇总：它们面向固定F2结果，顶层存在模型/数据导入或原目录写入，不能直接作为本批纯CPU入口导入。隔离原型`statistics_cpu.py`沿用既有episodes/requests/success/end_status合同，仅增加显式快照适配和缺失/冲突校验；不替换线上汇总器，不建立竞争生产入口。

38项unittest全部通过（B计数/聚合/公式/簇/兼容27项，C静态6项，D预算5项）。覆盖成功/策略失败、基础设施有限重试、未执行/持续缺失、完全重复幂等、冲突completed报错、跨模型/训练seed/服务评测版本隔离、无作用seed别名去重、回合/请求/启动探针分母、A合法回退次数与代价、缺raw/detail保留失败、空配置不重分权重、共同支持，以及unknown/不支持簇结构拒绝。重采样仅合成已知簇，最多每次50次；同簇四组和clean/七维共享抽样，训练seed分开，固定seed和行逆序可复现。单簇不输出可靠区间，不运行真实bootstrap或p值。

| 合成手算例 | 预期 | 实际 |
|---|---:|---:|
| 配置A为1/1、B为0/9，配置等权 | 50%（非池化10%） | 50% |
| 60/70/70/75%的四组R，I_AB | −5个百分点 | −5个百分点（浮点舍入） |
| S的C/R=60/40%，增强=80/60%，G_drop | 0个百分点 | 约−1.11×10⁻¹⁴个百分点（舍入） |
| 七维缺一维 | 完整R不可计算 | null，缺失维度保留 |
| SAB两样本方向一条invalid，动作[2,4]、方向[10,20]、B[.2,.4]，λ=.3/.1 | action=3、direction=10、B=.3、total=6.03 | 一致；invalid只改A打包 |

只读兼容检查复用SA full-A、SB修复版、S-500各20回合，分别复核26/31/24请求，与原记录一致；未生成新成绩、未合并版本。回合JSON中的raw token缺字段不代表独立服务原件不存在，标注“本记录缺失/未推断”；SA具体decode_detail缺字段也保留，不能删掉真实失败。旧初态hash不足以构造正式有效单元，三份旧记录未送入正式统计。

**C：静态配置。** `static_budget.py`使用已有`xiyuan-ab-real-entry-config-v1`形状，四份synthetic配置与允许差异表留`config-fixtures.json`，不是可运行作业。公共base/LoRA身份、数据/norm/tokenizer/动作合同、seed/schedule、batch/累计、K/LR等按共同字段或明确draft侧表检查；现有resolved config未嵌入全部基础身份的缺口不补造。关闭分支不要求资源，启用分支缺身份报错，推理监督依赖必须为空；λA/λB=0不等于结构关闭或无需读取。实际SAB、梯度、参数更新、前向和推理隔离均未在本批验证。

**D：参数化预算。** `budget-draft.json`从候选条目计算四组×3 seed为18,924个候选模型回合，尚非冻结有效回合数。按各组不同步时、K、seed和占卡数计算；已为GPU秒的不重复乘卡数，已含开销不能重复加，同卡service/renderer要求按占用区间并集计；缺失/零吞吐和请求延迟不能冒充端到端回合吞吐。保留S-500完成窗口原件作历史来源，不把其含诊断/评测总窗口当稳态步时；不重测S-full。

纯假设示例：S/SA/SB/SAB分别30/32/32/36秒每更新、K20k、各3 seed、端到端120回合/GPU小时、另加未包含开销70卡时，则训练约2166.67、评测157.70、合计2394.37 GPU小时；4卡×70%对应约35.63天资源下界，非完工承诺。全部未来吞吐均明确假设，未测SAB/真实长回合速率输入unknown的情景总预算和天数均为null，不用0替代。参数和卡数不构成任何作业授权。

**实际执行、资源与结束位置。** 本批主命令为`env OMP_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 MKL_NUM_THREADS=2 CUDA_VISIBLE_DEVICES='' PYTHONDONTWRITEBYTECODE=1 python3 <CPU_PREP>/run_checks.py`，退出0；此处路径为公开占位，实际绝对命令留registration。仅加载显式`test_cpu`，未收集全仓库测试。最终单进程验证约1.04秒、峰值RSS66,776 KiB，读取约14.99 MB既有小型来源，地址空间硬上限2 GiB，GPU作业0；前期原型测试与首次候选生成也为有界单CPU进程，没有安装依赖。`tests-final.log`、`verification.json`、各工具源码/hash和候选/兼容/预算原件保留本机，新增代码尚未合入活动工程，不能称工程提交已更新。

结束时仅做约定的原作业只读核对：2026-09-17 15:54（+08:00）原训练进程存活、心跳持续RUNNING；进度快照留本机，本批不改训练方维护的顶部步号和run记录。没有对原作业发送信号、修改线程/环境或启动替代作业；未留下本批后台分析/监控/队列。保护文件摘要未变，未观察到需停分析的资源争用证据，不据此声称测得零性能影响。
...
### 2026-09-18｜S-full step-2000 checkpoint 与固定诊断完成

S-full 单一 runner 已完成第 2000 次有效更新，诊断完成后 heartbeat 已续接到 2003，状态仍为 `RUNNING`；没有重启、第二个写入者或配置变更。第 2000 步 checkpoint 存在 `_CHECKPOINT_METADATA`，元数据 SHA256=`00b3aac96a620e04805ab952fe65180ad853ab493bf5ed0dc5b75e0ab396b294`，路径为 `runs/pilot/f2-sfull-coverage-20260917-s0/checkpoints/step-2000/`。
...
### 2026-09-18｜S-full实际曝光与460请求复算；SA-full/SB-full计划待审

依据负责人对`2cbe08cce1bb22612dc85eb827a3ba59c0559c8a`的审阅，本批仅CPU读取既有原件并细化F2计划。未新增模型计算、GPU作业、更新、仿真或旧评测重跑。复算脚本和完整回执位于既有审计目录`sfull-v1/completed_cpu_audit.py`及`completed-cpu-audit.json`；脱敏结果并入[聚合JSON](review/final-evidence/aggregates.json)的`sfull_coverage_control.completed_cpu_audit`，绑定输入和脚本SHA256。首次脚本把48000扁平schedule误作3000行比较而断言失败，修正比较形状后通过；没有改变schedule、metrics或模型。

**实际训练对应。** 原metrics确为3000行、有效步1—3000无缺失重复，每步16 ID；展开后的48000 ID与原schedule逐位置精确相同，全部已完成更新的真实曝光计数如下。保留`planned_unique_starts`原义，本次实际值另存`actual_coverage`；没有仅凭heartbeat到终点推断。

| 有效步 | 实际曝光位置 | 唯一动作起点 | 唯一episode | 固定train探针曝光次数（既定顺序） |
|---|---:|---:|---:|---|
| 1000 | 16000 | 13902 | 450 | 1,2,0,0,1,0,1,0,0,0 |
| 2000 | 32000 | 24245 | 450 | 1,2,0,0,1,1,1,0,0,0 |
| 3000 | 48000 | 32029 | 450 | 2,2,3,0,1,1,2,0,1,0 |

各时点十个val探针曝光均为0；探针顺序及各sample_id见JSON，不将train划分内但未抽到的探针称已见。实际覆盖与预登记计划计数一致。32029个起点不是覆盖完55682全集，扩大覆盖同时改变重复频率、时间位置和动作内容分布。

**回合与请求。** 20回合中460条关联请求与460条服务原始记录按request_index一一对应，无重复/遗漏，状态、decode_detail及指令hash精确对应。450/460请求合法（97.83%），10/460系数长度错误（2.17%）；没有本批dtype/边界错误。首请求失败0/20，10个解码终止全部发生在合法动作后；9/20成功、1/20超时，实际执行2233模型动作步（不含预热），不能以450×5代替因成功而提前结束的最后动作块。

| 任务索引 | row0 / row1结局 | 成功/2 | 解码失败：row→首错请求序号 / 已执行步数 |
|---|---|---:|---|
| 0 | 长度 / 长度 | 0 | 0→11/50；1→13/60 |
| 1 | 长度 / 成功 | 1 | 0→25/120 |
| 2 | 成功 / 长度 | 1 | 1→18/85 |
| 3 | 成功 / 成功 | 2 | — |
| 4 | 长度 / 成功 | 1 | 0→22/105 |
...
| 6 | 成功 / 成功 | 2 | — |
| 7 | 长度 / 成功 | 1 | 0→43/210 |
| 8 | 长度 / 长度 | 0 | 0→32/155；1→18/85 |
| 9 | 超时 / 长度 | 0 | 1→36/175；row0执行220步超时 |

任务索引沿原Spatial10清单，不重新排序；20条逐回合连续合法请求、首错误、实际步数均可从聚合域定位。解码失败前连续合法请求10—42次，执行50—210步；成功覆盖任务1—7，共7个任务，row0成功3/10、row1成功6/10。所有20回合均至少执行50步。请求有关联，97.83%不等于任务成功或独立可靠性样本。10个预指定row0视频仍保留原件，本批没有重新渲染、挑选或声称人工观看。

**判读与下一步。** 同初始化、预算和任务抽样序列下，S-full相较S-500的val生成与clean控制改善，支持任务内覆盖干预在该配置下有帮助；尚余10个格式终止和1超时，不能称稳定控制已达成，不能外推A/B全数据效应。首选下一批仅SA-full与SB-full，从基础初始化直接复用本次完整schedule，沿用监督/缓存和原辅助日程；各3000步、三点固定诊断、终点各20clean，最多两张空卡/每组30卡时/合计60为待审提案。本批未建立或启动队列。

PLAN已明确受影响入口检查、A invalid保留动作、SB读取隔离、按预算停止，以及新增待审的持续动作/跨任务成功量化判据。阈值不是原协议旧要求，也不要求超过9/20；批准前不执行，执行后仍须明确F2技术结论。SA长回合方向开销可能使30卡时不足，计划按真实路径估余量并预留退出，不能拿S的24.35小时保证两组同速。既有失败、服务PASS、QC采用/human_reviewed=0、独立备份未确认均保持。


### 2026-09-18｜SA-full/SB-full批复采纳、真实CPU连接与预算停止启动

已完整读取负责人转发对3c71cb4的批复。方案和数字功能最低线已采用，不重新提交同一方案审批；仅预算可行且前检通过的组可启动。持续执行按实际环境步数（排除预热）与连续合法动作请求共同判断，提前成功计合格；SA方向/回退为请求内部过程、不重复计数，另报回退请求和成功回合是否回退。所有阈值是本次新增开发最低线，不是部署可靠性或方法收益结论。F2不自动DONE，F3/SAB未获授权。

**本轮实际执行。** 只在既有解码审计目录增加CPU前检源码`full-ab-preflight.py`和各组回执；调用共用入口`_open_real_inputs`、`_real_microbatch`，JAX_PLATFORMS=cpu、CUDA_VISIBLE_DEVICES为空，OMP/BLAS线程2，无模型加载、无GPU运算或更新。共享训练/服务源码未修改。两组均使用原完整55682池和同一外部schedule，shape=3000×16，hash=dd0bcd1a…；SA完整标签版本及approval被真实入口接受，48000位置中45377带方向、2623 action-only，valid/invalid混合batch动作监督均非空，invalid方向mask为0。SB缓存合同6de0a757…与全manifest连接通过，首有效批16个真实样本按4×4读取得到(4,2,256,2048)目标及有效mask，无方向文件读取；SA无教师payload读取。未重新生成任何监督/缓存。

首次CPU断言错误要求SA辅助占位目标为None，而原共用入口返回零目标/全false mask；核对源码后改为检查原合同的零占位与无教师读取，复测通过，未改变生产实现。随后用首三个固定有效批做CPU输入预算检查，不执行模型；同ID先S再对应variant，因此源数据热缓存效应单列，三批只是小规模预算证据。两个前检最终退出0。初始化的实际GPU数组核对尚未运行，不写PASS。

**预算前检（不是GPU实测）。** 原S-full全3000更新平均28.6736秒/步、末500平均30.3013秒/步；旧SA/SB恢复段56781.24/62387.40秒对应2000更新，约28.39/31.19秒每步的进程段平均，并非稳态纯计算。不能只引用26.184/28.589短测。SB三个固定批的CPU输入时间分别6.480/12.433/12.388秒，同ID S为0.073/1.215/0.076秒，增量6.407/11.217/12.312秒，中位11.217秒。该读取与打包在现入口求导前完成；保守串行外推不冒充最终端到端耗时，不声称三个批代表全部缓存分布，也未加入额外B计算成本。

| 组 | 预计训练 | 三点诊断 | 完整20回合 | 加载/保存/退出及余量 | 合计/超30小时 |
|---|---:|---:|---:|---:|---:|
| SA | 25.251小时（S-full末500步实测外推，仍非SA实测） | 0.367小时（60×22秒情景） | 5.378小时（880×22秒长回合情景） | 0.5小时假设 | 31.496 / +1.496小时 |
| SB | 33.243小时（S-full全程步时+CPU输入增量中位外推） | 与评测/退出合并保留 | 与诊断合并保留 | 共4小时最低保留，未冒充分别实测 | 37.243 / +7.243小时 |

预算依据与原始三批时间、来源SHA存`full-ab-budget-precheck.json`，脱敏副本在聚合`full_ab_validation_precheck`独立域。上述数字是启动可行性情景，不是实际新增GPU账单；本轮新增GPU成本0。SA可能超限、SB当前读取路径也没有足够余量，故按本次批复“预计明显超过30小时先报告差额”两组均不启动，未创建训练队列、不缩短评测、不借额度。没有将预算问题计为模型失败或撤销方案许可。

下一项需要技术决定的是预算可行性：优先审阅SB实际读取增量及可复用的确定性输入缓存是否值得做有限工程优化，再定是否能在原30小时完成；SA同时需确认方向长回合保留量。当前未采用任何缓存优化、未加GPU试跑或延长上限。已有服务、QC、初始化历史证据及原模型均保留；本批尚无新run/恢复点，S-full原1000/2000/3000不动。


### 2026-09-19｜SB读取检查收口采用原版；统一启动前发现SA方向异常分支缺口

**授权与现场。** 通过用户指定ChatGPT分享页取得完整交接正文，按先检查SB、后两组同批启动执行；SA34/SB42/总76卡时替代旧30/30/60，仅资源上限改变，科学合同和功能最低线不变。文档起点6602762、工作树起初干净；实际工程HEAD为16295beccf737e1e180718fe78af963cd8707999，已有gemma_fast.py dirty及未跟踪AGENTS保留。一次宿主只读进程核对未发现本项目新训练/评测进程；S/S-full/旧SA/SB目录只读保留，未重新核验全量checkpoint或教师payload。

**隔离和真实调用。** 原reader及入口字节快照保留在已有解码审计目录reader-first-v1，开始registration记录2小时截止；只新增本机技术产物。CPU进程显式CUDA_VISIBLE_DEVICES为空/JAX_PLATFORMS=cpu，OMP/BLAS/MKL线程上限2，RLIMIT_DATA=8GiB，实测峰值RSS约1.82GiB；reader沿原2片LRU，约80MiB持久payload缓存（get_many同批临时持有另计），未使用4GiB缓存额度、未清系统页缓存。没有候选补丁，没有GPU回归、依赖安装或全局配置更改。完整原入口与reader开始/结束hash相同。

profile_reader.py调用实际_open_real_inputs一次、实际_real_microbatch按4×4批量、ProductionTeacherCache.get_many；一实例顺序读取预先固定12个更新号两轮，合计384个样本位置/96微批次。原schedule及顺序未变，无训练状态/RNG消费。带cProfile和计时wrapper，分项嵌套且有instrumentation开销，不与原无profiler三批精确相减，不把分项机械相加。

| 原reader测量 | 首次序列12批 | 同实例重复同序列12批 |
|---|---:|---:|
| 完整输入平均秒/16样本批 | 11.0868 | 2.6714 |
| hash调用次数 | 188 | 0 |
| payload载入次数 | 191 | 191 |
| hash累计秒 | 60.3056 | 0 |
| NPZ成员载入累计秒 | 10.9136 | 5.3759 |
| finite检查累计秒（含调用链其他检查） | 9.0909 | 8.7898 |
| norm累计秒（含训练目标检查） | 13.6830 | 3.9238 |

**结论范围。** reader/索引/verified集合生命周期是run级，不是每微批次重建。production_reader.py:_validate_shard首次对分片hash，缓存淘汰后verified仍存在；get_many每次微批内部对同片去重，但LRU仅2片，跨随机请求通常再载入。实际zip成员compression=0，即ZIP_STORED，无压缩解码；20行分片的目标FP16数组41,943,040字节，单行2,097,152字节。典型16不同片批次载入目标约640MiB，仅返回32MiB；4样本返回(4,2,256,2048)/FP16、8MiB目标，mask独立。返回字节、hash逻辑访问字节和payload处理字节均不等同物理磁盘流量，物理I/O为unknown。

cProfile真实调用总165.165秒，get_many累计145.301秒，_validate_shard累计145.157秒；首次hash中的read累计53.344秒，说明首次完整性读取有实质等待/读取成本；不能进一步无证据归因为NFS故障。重复轮没有hash但仍整片载入/astype/finite/norm，hash缓存与payload缓存效果不同。全48000位置纯metadata模拟（每微批同片去重）47980次片访问：2片LRU命中10、未命中47970；不能把重复12批效果当作整个训练的缓存命中率。

**采用决定：继续原reader。** 该固定范围未发现reader正确性错误，首次校验必须保留；单纯增加小LRU的实际命中收益缺证据，本轮不实现候选、不做多方案搜索。未改输入端，不需要再做“候选等价/候选恢复”或GPU数学回归，引用原已通过reader错误/顺序/恢复证据，不扩称全部新错误fixture已重跑。两轮检查在2小时内结束，真实耗时、逐批计数、profile、内存、source hash在decision.json及original-profile.json；本轮GPU=0。

**新预算。** SA情景32.20小时：训练25.30（S-full末500实测30.3013秒/步外推）、60自然生成0.37（历史22秒/请求情景）、6批TF0.40（保守假设）、完整880请求闭环5.38（同历史情景）、加载/保存/视频及退出0.75（假设）。SB保留旧37.24小时保守含输入情景，再单列额外B计算2.50小时假设（3秒/更新）及首次校验/TF余量1小时，共40.74小时；不再次把完整CPU输入时间叠加到已含输入的旧情景。SB上述保留量有保守重叠，不是精确成本上界或新GPU benchmark；首次hash只记首次项，不能三千步重复套11秒。两组无可靠证据显示34/42必然不可行，原本可进入同run前100步校准，但以下正确性缺口阻止本次统一启动。

**新发现，具体范围。** 启动前核对实际SA服务发现serve_sa_full_a_dev.py:104的Policy.infer直接调用_constrained_direction_generation，方向候选解析及预算异常无捕获/重新S前缀路径。helper在test_a_real.py:144会明确raise方向预算耗尽；WebSocket通用异常处理不是action-only回退。check_sa_error_path.py抽取真实Policy AST并注入synthetic预算异常，零模型/零GPU：只发生一次带问题前缀的inference_batch，随后异常上抛，action-only sample_actions调用为0。真实源码SHA256=d635be1fdb77b64a6a283ee8fb2bdf2377f7a15a95bcee5ef6876c0943427a7d，复现回执sa-service-readiness.json。此测试只证明受控方向异常分支，不冒充真实模型生成或认定旧20回合曾触发该问题。

此外现服务正常返回未包含统一request_index/raw decode_detail/fallback标志，旧审计器另行捕获raw；本批要求正常/回退请求和成功回合分类，不能继续靠独立审计包装补全实际服务。这与原action-only token适配已修复是不同事项；旧full-A正常路径结果及回退工具层测试保持原范围有效。生产服务未修改，本批两组没有带已知合同缺口开训，也没有改成SB先训练。

**停止位置与下一动作。** SB检查已完成、原版采用已落盘；统一启动前还需在实际SA公共服务接通规定的方向错误重新prefill及统一请求审计字段，并做受影响分支回归。此次交接的窄范围实施修改限于输入端，故先报告该服务实现差异，不擅自扩成新推理器或新模型实验。修复不能增加动作错误重试，也不能改变正常方向—动作数学路径、预算或旧模型。当前无新run/新checkpoint，无GPU占用等待；F2保持IN_PROGRESS，F3/SAB未启动。后续须沿本次采用reader和34/42额度，不重做同一批读取测速。


### 2026-09-19｜SA运行时回退修复、CPU通过与8请求GPU回归的明确缺口

完整读取负责人链接补充批复，明确允许真实SA服务控制流/请求日志/客户端关联修复；SB reader检查不重开。旧源码、实际输入和日志快照留既有审计目录sa-runtime-fix-v1。两组新训练在本批回归前后均未启动，F2未验收，F3/SAB未授权。

**实际修改。** test_a_real._constrained_direction_generation只将原方向无候选/预算耗尽异常改为带部分token的DirectionGenerationError；选token、循环、末token消费、KV位置和32/256容量不改。真实serve_sa_full_a_dev.Policy现在为可复用顶层类，三点诊断和WebSocket共用；仅捕获方向helper类型错误、候选解析DirectionDecodeError以及公开文本helper的明确空指令错误，不吞模型/输入/显存错误。异常处理退出后丢弃方向状态，使用同一SA模型及原始公共观测/指令重新pipeline.inference_batch，再走公共S的固定256/温度0动作调用。动作解码在方向异常捕获区之外，无正常/回退动作失败重试。

SA真实服务改用现有公共选择性LoRA加载器，实际服务10个共享参数叶子逐项核对，teacher/projector/方向文件不加载；旧完整checkpoint/optimizer加载不再用于推理。方向raw/具体错误/消耗预算、正常或fallback路径、动作raw及严格decode_detail、host关联和总耗时每逻辑请求一个最终记录；模型或其他异常仅记录后原样抛出，日志失败不触发回退。SB正常动作计算未改，仅过滤公共输入并增加“不适用方向”、关联和耗时记录。客户端_audit仅含run/episode/step/attempt，在服务传给模型前剥离；回合记录保存SA方向/回退字段。注入仅在隔离GPU测试包装，生产没有强制回退选项。

**CPU真实分支。** 实际服务Policy的11项synthetic测试通过，含合法方向、真实helper恰第32token完成/到限不完整、部分方向预算异常、候选/文本解析异常、正常动作错误不回退、回退动作错误不二次尝试、模型/输入异常传播、日志失败传播、回退后正常请求计数及缓存隔离。真实helper使用小型synthetic模型张量验证边界，未加载权重；原缺失分支的失败回执保留。没有改FAST、标签/QC或teacher payload。源码快照、测试及cpu-final.log可追溯，未推送私有代码。

**真实GPU/WebSocket。** 同一既有SA step-3000、容量144、固定val索引0和591，公开输入/key0/温度0。由实际选择性服务加载器核对对应checkpoint的10个LoRA叶子；旧正常Policy从旧源码提取，修复服务走实际Policy，正常路径均实际GPU生成。故障是隔离包装受控方向预算注入，回退prefill与动作生成真实在GPU执行；同一对象、权重和输入的direct公共S动作路径用于参照，不换成S-full权重。

第一轮前两请求完成val0旧/新正常路径；第三请求回退时因显式max_decoding_steps关键字被module_jit当动态量而触发ConcretizationTypeError，未形成动作结果。错误留gpu-server.log/RAISED请求原件。已改为公共S服务相同的固定默认调用（源码默认256/temperature0），没有变预算或编译边界；CPU11项复测通过。该失败仍占一次请求额度。

原批最多8个请求，因此剩余只执行5个：val0修复回退、val0 direct、val591旧正常、val591新正常、val591修复回退。两输入正常生成raw token数值/动作及严格失败结果、真实模型输入指纹精确一致；val0回退与direct精确一致，二者都真实返回71系数长度错误（未修成70）；val591正常旧/新和修复回退均严格合法70系数。

第二轮比较脚本曾要求整个decode_detail字典相等而失败：旧路径观察器把raw转Python列表再np.asarray重建为float64，新真实输出为float32。该dtype字段是测试观察器产物，旧生成数组dtype没有独立存下，不能伪造补录；raw数值、shape、其余decode字段、动作和实际输入指纹精确一致，已在CPU对已保存结果逐项复算，保留原AssertionError。不以该观察器dtype假差异认定模型路径改变，也不声明旧输出dtype已被直接核验。

| 固定输入 | 旧/新正常对照 | 真实回退vs同权重direct | 当前范围 |
|---|---|---|---|
| val0 | token数值/动作结果/输入一致 | 一致，双方71系数失败 | 所测正常和回退对照成立 |
| val591 | token数值/动作结果/输入一致 | 回退已真实执行且70系数合法，direct未执行 | 缺同输入direct参照，不标完整通过 |

**额度与停止。** 8请求尝试=1次失败+7次返回；零训练更新、零机器人闭环。两次GPU占用44.1618+45.1159=89.2777秒（0.0248 GPU小时），计入SA34；不是耗完1卡时，而是请求数额度已用完。服务已按runner finally退出，宿主定点进程核对无遗留回归服务、新训练未启动。全部回执、逐请求和代码hash并入独立sa_runtime_fix_20260919聚合域，reader_first历史不改。

**唯一缺少的最小验证。** 需额外允许1次val591、同SA step-3000/144/key0/固定256公共action-only WebSocket请求，与本轮已保存的该输入修复回退结果比较；不重复旧/新正常或其他7次返回，不新增闭环/训练，不改其余协议，额外加载与该请求继续计入SA34及服务回归累计1卡时。当前未执行第9次，未把局部对照或合法动作当成完整回归通过。按补充批复“未通过时两组保持未开训”，本轮停在此可审阅位置。

启动worker的复用草稿留本机full-ab-v1，未运行、未登记为已验证runner，不作为完成证据；训练数学源码和reader未改。F2仍IN_PROGRESS，待补齐实际服务回归后才可按既有授权统一启动两组，不恢复F3。


### 2026-09-19｜第九参照补齐通过；SA-full/SB-full统一启动

负责人新批复只增加val591一个direct action-only WebSocket请求，总上限由8改9，原GPU累计上限不变。已用原SA step-3000、144、key0、默认固定256/温度0执行，无额外自然生成/预热请求；实际服务加载10个LoRA叶子核对通过，不读teacher/projector/方向文件。新增结果真实raw dtype=float32；与已保存val591修复回退的真实输入指纹、raw token、全部decode_detail、动作数组精确一致，不仅比较合法70系数。旧正常路径原始dtype未直接保存限制保留。

第九次含加载/编译/请求/退出占用21.0007秒；加旧89.2777秒，累计110.2785秒（0.030633 GPU小时），计入SA34。该固定两输入服务回归完成；原8次失败/局部结果保留，追加ninth-reference/comparison.json和独立聚合域，不回填旧FAILED为PASS。GPU服务已退出，未新增机器人闭环或训练更新来完成回归。

**启动检查。** 原SB reader及训练入口不变，CPU标签/cache连接与schedule身份复用此前通过回执。共用训练数学入口ab_training_entry.py hash仍d2b3bf09…；独立初始化key101与projector key107保持。启动worker复用已执行S-full的训练/保存观察回调及顺序评测流程，参数化为SA/SB；增加真实输入计时、每100步按完整均值/P50/P90预测预算、预算不足驱动既有完整保存退出。CPU预算fixture检验继续/超额暂停/3000步剩余项，源码语法和前检hash对应通过；这些不是实际新模型更新或恢复PASS。

实际生产服务、方向helper/严格适配、reader、Gemma补丁、训练worker/runner及预算函数的hash固定于full-ab-v1/adopted-sources.json；checkpoint诊断前与终点评测前检查保护版本，不热改运行中源码。生产没有故障注入开关。三点自然生成调用实际SA Policy或公共action-only Policy；TF独立构造，SB无方向读取，SA按valid/action-only分别记录。原模型推理/训练参数和RNG不因计时改变。

已核对两张兼容RTX A6000空闲后，同批启动两个唯一run：runs/pilot/f2-sa-full-20260919-s0与runs/pilot/f2-sb-full-20260919-s0；各自runner/精确GPU/PID/deadline保留orchestration.json，不公开服务器标识。本节点仅确认runner启动，实际初始化与首更新待原件确认，不能提前写训练通过。SA34小时已扣服务回归110.2785秒，训练前保留6.7小时；SB42小时保留4小时，合计76不互借。前100步预测用已发生占用、剩余更新完整均值、未完成诊断/评测/退出保留；明显超额则由共用入口保存最新完整状态并停止，不自动延时。

后续正常流程由本批runner执行3000步、1000/2000/3000固定20观测+2批TF、终点原20clean及预指定row0视频；不挑checkpoint、不重复策略失败。当前F2仍IN_PROGRESS，结果达最低线后仍须技术审阅，F3/SAB不启动。首更新、100步与恢复点按真实证据续记。


启动后实据追加：两组initialization-check.json均核对10个LoRA叶子及全部初始参数与归档代码重建对象精确一致，共享初值指纹2f86409ca081f3a222e1fe652ffd2de6e833927e1e4e43b8aa9ea3223259ac1a；重建证据不冒充旧数组原件。两组首3步实际sample_id逐位置与固定schedule相同，动作有效token非空，所有损失及梯度范数有限，共用入口冻结不变/LoRA更新断言均已执行。SA动作loss17.46875/15.5/16.125，方向loss13.046875/12.8125/13.8125；SB动作loss14.953125/13.0625/14.25，对齐loss1.00164947/0.99871808/0.99921232。SA无对齐，SB无方向，不比较不同目标的总loss排名。

首3步完整更新时间SA29.06/24.72/24.86秒、SB39.29/73.99/27.16秒；含起步波动，不能用最快值承诺全程。各自startup-verification.json留原件，聚合仅脱敏指标与来源；第100步校准尚未产生，后续由已启动runner记录并依实际预算继续或完整保存退出。新run尚无1000步磁盘恢复点，不把首步heartbeat写成checkpoint。本批初始化、首次编译和占卡等待全部计入各组额度，无重新起算。


### 2026-09-19｜两组前100步预算校准认可并继续

承接负责人对bcc7e45启动节点的认可，未重复服务回归、CPU连接、reader或初始化测试。两条真实运行句柄仍存活，原runner已自动完成第100步校准并继续；本轮读取已产生metrics、input-timing与calibration-100.json，前100有效步1—100连续、1600个sample_id逐位置匹配原固定schedule；动作有效token非空、loss/梯度范数有限，每步4个微批次。冻结/LoRA更新由原入口逐步断言，本轮没有额外参数前向或训练。

| 第100步实测/预测 | SA-full | SB-full |
|---|---:|---:|
| 完整更新均值 秒 | 25.2251 | 32.0954 |
| P50 / P90 秒 | 24.8449 / 26.3068 | 31.4124 / 34.5609 |
| 输入准备host均值 秒/有效更新 | 0.8960 | 7.5288 |
| 当时GPU显存快照 MiB | 17289 | 17295 |
| 当时已计占用 卡时 | 0.8417 | 0.9679 |
| 预计整批 卡时 | 27.8620 | 30.8225 |
| 相对34/42卡时上限余量 | 6.1380 | 11.1775 |
| runner决定 | CONTINUE | CONTINUE |

完整更新时间来自已有实际参数更新、指纹校验和指标完成后的落盘计时，含输入；输入列为_real_microbatch周围host耗时，包含数据、编码打包、SB缓存和device提交，不能再加到完整步时上，也不能直接称为纯教师读取时间。加载、初始化重建和首次编译未单独完整计时，分别耗时保留unknown；它们已包含在实际占用或首更新中，不漏掉、不补造分项。显存为第100步单次快照，非峰值。

预测采用已发生占用＋剩余2900更新×完整均值＋未完成诊断/评测/保存退出保留；SA原6.7小时、SB原4小时保留仍在预测内。SA服务回归110.2785秒在既有deadline扣账中只计一次，不另加一遍。此表是第100步窗口外推，非实际终点成本或完工保证；初期慢步骤进入均值/P90，不据此调参或另开测速。预算/正确性正常即继续，无需新许可；之后每100步校准或实际异常由原机制判断，预计超额完整保存并停受影响组，无共享问题的另一组可继续。

来源为各run的calibration-100.json、前100行metrics/input-timing；公开摘要及calibration文件SHA在聚合full_ab_20260919各组calibration_100域。原日志、9次服务请求和历史reader结果不变。PLAN旧“缺第九次/未启动”条目标注历史取代，不作为未执行任务，也未删除失败历史。下一集中节点为1000步完整保存与固定诊断；3000步/终点20clean和功能最低线不变，F2未验收。

### 2026-09-19｜SA-full/SB-full训练因GPU设备访问异常暂停

在收到持续监测请求后对实际运行目录做只读复核。首次读数为SA=282、SB=236；短间隔内两个run文件继续写入，最终观察到SA `effective_update=288`、SB `effective_update=241`，随后约一分钟未再推进。两组各自的 `result-heartbeat.json` 字段仍为历史 `status=RUNNING`。本会话的 `ps` 未发现对应训练进程或第二写入者，两个run目录均没有 `checkpoints/step-*` 目录，因此没有可宣称的完整恢复点或1000步保存点。已形成的初始化、首3步、100/200步校准和1—288/1—241指标原件全部保留。

同一只读核对中，当前执行节点没有 `/dev/nvidia*` 设备节点，`nvidia-smi` 返回无法与NVIDIA驱动通信；普通权限无法读取内核日志，故只登记为“GPU设备/驱动访问异常”，不臆测宿主根因。未发送进程信号、未删除运行目录、未重启训练、未改参数或预算。此次停止不是模型数值或数据合同失败的结论；未完成checkpoint、诊断和终点评测均保持未执行状态。

后续恢复前必须先确认GPU设备可见且有唯一写入者，再决定是从已有可恢复点续跑还是按保留的部分run重新开始；当前没有可用checkpoint，不能把282/236伪装成可恢复步。SA34/SB42/总76 GPU小时上限、3000步目标、固定诊断和终点评测合同不变。F2保持IN_PROGRESS，F3/SAB不启动。

**更正说明。**上一段初始记录的282/236是第一次异常快照，不是最终文件步数；后续短时写入到288/241后停止。该更正只修正现场观测，不改变“无checkpoint、GPU访问异常、不得自动重启”的决定。
