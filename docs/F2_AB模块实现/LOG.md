...
2026-09-19当前节点：SA-full/SB-full在前100步预算校准后继续运行，但最新现场显示两条训练进程均已退出，未形成1000步checkpoint。最后实际完成步数为SA=282、SB=236；两份result-heartbeat仍停留在RUNNING，不能当作活跃状态。当前节点已标记为GPU访问异常暂停，F2仍未验收，F3/SAB未授权；不在GPU设备恢复前重启或创建第二写入者。最近完整磁盘checkpoint须以实际保存节点为准，不能把heartbeat当恢复点。
...
在收到持续监测请求后对实际运行目录做只读复核。SA run 的 `result-metrics.jsonl` 最后一条为 `effective_update=282`，SB 最后一条为 `effective_update=236`；两组各自的 `result-heartbeat.json` 最后更新时间分别对应这两条记录，但字段仍为历史 `status=RUNNING`。现场 `ps` 未发现对应训练进程或第二写入者，两个run目录均没有 `checkpoints/step-*` 目录，因此没有可宣称的完整恢复点或1000步保存点。已形成的初始化、首3步、100/200步校准和1—282/1—236指标原件全部保留。
