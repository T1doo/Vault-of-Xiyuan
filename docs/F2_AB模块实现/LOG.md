...
最后核对：2026-09-15（服务修复及版本化重评完成）。**F2=DONE（独立工程实现、监督/缓存、小规模训练与规定评测的限定范围）；稳定控制目标未达成。G1保持PASS，G2未通过，不进入F3。** 依据负责人本轮明确授权，真实服务修复、同权重端到端回归、SB原20单元一次重评均已完成，无需再次申请相同限定范围的收尾。

实际action-only服务已在`Policy.infer`调用链复用F1经验证的`generated_token_ids`；不放宽严格FAST，不改变模型输出。CPU非法输入拒绝和真实错误保留通过。同一F1 step-3000、144容量、val索引0/591的旧/新真实服务经WebSocket自然生成，实际参数、预处理输入、位置/注意力、首个logits、原始token、解码动作均精确一致。真正SB服务中10个LoRA参数叶子逐一对应原step-3000数组，仅选择性恢复LoRA；不加载projector、教师缓存或方向标签。

当前有效开发结果：SA full-A沿用原20回合、26请求、6合法、20长度失败，成功0/20；SB修复版原20单元重评共31请求、11合法、18系数长度错误+2边界错误，累计执行55步，成功0/20。两份旧action-only 0/20仍为受服务缺陷影响的历史运行，SB旧版不参与模型效果统计，SA额外诊断不重评。新旧结果不混成40回合，也不逐回合挑选。SA full-A及其适配helper字节未改，无需重评。

两组3,000步训练、六个checkpoint、已采用A标签/300题GPT技术复核、完整教师缓存及性能优化全部保留。`gpt_technical_reviewed=300`、`human_reviewed=0`不变。原同NFS保护副本保留，本轮另校验45成员增量源码/请求/回归副本；独立备份仍未确认。

本轮回归及启动核验约149.09 GPU秒，低于1 GPU小时上限；重评单列58.19秒，含服务退出等全部GPU进程区间合计207.75秒（约0.05771 GPU小时）。均为进程占用墙钟，非kernel计算时间。初次SB加载失败在任何回合前退出，错误保留；本轮没有训练更新、动作失败重试或第二轮完整重评。回归/评测服务已释放。

下一阶段只登记待办：500池代表性及控制能力有限；SAB组合、正式四组初始化/服务/数据/预算、真实评测清单/统计/成本仍待下一轮F3设计与G2。当前停在F2，不启动F3计算或正式作业。具体版本、回归证据、逐请求分母与限定收尾采用说明见本日志末节。
