...
最后核对：2026-09-15（终点补核）。**G1=PASS；F2=BLOCKED（实际action-only服务类型适配缺失），不满足本轮限定性收尾条件。** 两组3,000步训练和三批评测执行记录保留，控制目标未达成；本轮不训练、不重评、不进入F3。

本轮从真实服务代码发现：`serve_ab_action_only_dev.py`将float32容器中的整数token直接传给只接受整数dtype的严格解码器，且返回通用`FastDecodeError`。F1旧服务有`generated_token_ids`适配。重放两个已保存、来自同一F1 checkpoint的合法输出，在同一公开输入/144容量下，旧包装2/2成功、新包装2/2因`invalid_token_shape_or_dtype`失败。此为CPU在模型输出边界的真实包装重放，不是完整同权重GPU前向或四次新自然生成。第一个具体差异已定位，停止扩量。上轮审计器自行做了int32转换，其结果不能证明原action-only服务正确；上轮“没有共同服务错误”的推断撤回。

恢复位置：SA/SB各自原step-1000/2000/3000六个checkpoint本轮逐文件hash核对，保留旧恢复证据；不将元数据hash冒充模型全参数校验。代码、配置、metrics、标签采用记录、审计及六个checkpoint已保存315个成员的同NFS保护副本并校验内容；独立备份未确认。精确位置与hash见本机`final-check/preservation-index.json`。

成本纠正：约56,793/62,398秒属于恢复启动器区间；训练进程heartbeat记录56,781.238/62,387.397秒，均对应1001—3000步及加载/保存/等待，非全训练或纯GPU计算成本。前1000步及暂停占卡成本不完整；现有metrics无逐步时间戳，不绘制伪造步时分布。本轮新增GPU成本为0。

监督采用保持`gpt_technical_reviewed=300`、`human_reviewed=0`，参考点/接触代理/2 mm死区和阶段unknown限制保留。本轮六表、三图与可复画聚合数据见本日志末节；历史三批结果分别保留，SB和额外SA action-only评测受服务缺陷影响，不能写成可靠模型负结果。full-A原20回合/26请求/6合法/20失败仍保留。

下一步是修复并验证已定位的action-only包装差异，完成真正同权重同配置端到端对照；是否重评按下一批范围处理。本批依停止边界不修改生产服务、不进行二十回合重评或重训，不把技术缺口移交成F3豁免，也不把本轮状态写成只待负责人签字。
